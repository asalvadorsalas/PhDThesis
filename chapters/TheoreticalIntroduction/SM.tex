...
On another note, the top quark is the most massive known particle known to date since its discovery in 1995. Such characteristic makes the top quark the only one that decays before hadronisation and an excellent candidate to study new particles, from much more massive ones that might decay to the top quark and then to other lighter exotic particles. Figure[]crossection?. The top quark is also used to measure possible FCNC processes, for which there is no direct coupling in the \acrshort{SMlabel} as explained in Section[]. Nevertheless, higher order processes involving $W^\pm$ are possible making FCNC processes very supressed. As the top quark can decay to any other \acrshort{SMlabel} particle, an observation of FCNC processes would imply the need to expand the \acrshort{SMlabel}. Table with measuremetns\\
